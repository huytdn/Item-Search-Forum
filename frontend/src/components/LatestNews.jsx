  ...
  return (
    <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 mt-10 relative">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <button className="text-indigo-600 border border-indigo-100 px-4 py-1.5 rounded-xl text-sm font-semibold hover:bg-indigo-50 transition-all">
          Xem thêm
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 transition-all duration-500">
        {currentPosts.map((post) => (
          <div
            key={post._id}
            className="border border-gray-100 rounded-2xl overflow-hidden hover:shadow-xl transition-all duration-300 bg-white flex flex-col h-full group"
          >
            {/* Image Section */}
            <div className="relative h-44 overflow-hidden">
              ...
              <div className="absolute bottom-3 left-3 flex gap-1.5">
                {/* Badge dựa trên type: lost (mất đồ) hoặc found (nhặt được) */}
                <span
            ...
            <div className="p-4 flex flex-col flex-1">
              <h3 className="font-bold text-gray-800 text-sm mb-2 line-clamp-1 uppercase tracking-wide">
                {post.title}
              </h3>
              {/* Sử dụng field description từ Model */}
              ...
              <div className="flex items-center justify-between pt-3 border-t border-gray-100">
                ...
                <button className="bg-indigo-50 text-indigo-600 text-[10px] px-3 py-2 rounded-lg font-bold hover:bg-indigo-600 hover:text-white transition-all duration-300">
                  Chi tiết →
                </button>
              </div>
            </div>
